...
<main class="mx-auto max-w-4xl px-6 py-10">
    ...
    <form method="POST" action="{{ $item->exists ? route('admin.books.update', $item) : route('admin.books.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label for="field-author_name" class="text-sm font-medium">المؤلف</label>
                <input id="field-author_name" name="author_name" value="{{ old('author_name', $book->author_name) }}" required class="mt-1 w-full rounded border-slate-300">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            <div>
                <label for="field-publisher" class="text-sm font-medium">دار النشر</label>
                <input id="field-publisher" name="publisher" value="{{ old('publisher', $book->publisher) }}" class="mt-1 w-full rounded border-slate-300">
                @error('publisher')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-publication_year" class="text-sm font-medium">سنة النشر</label>
                <input type="number" id="field-publication_year" name="publication_year" value="{{ old('publication_year', $book->publication_year) }}" class="mt-1 w-full rounded border-slate-300">
                @error('publication_year')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-pages_count" class="text-sm font-medium">عدد الصفحات</label>
                <input type="number" id="field-pages_count" name="pages_count" value="{{ old('pages_count', $book->pages_count) }}" class="mt-1 w-full rounded border-slate-300">
                @error('pages_count')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
        </div>

        <div>
            <label for="field-isbn" class="text-sm font-medium">ISBN</label>
            <input id="field-isbn" name="isbn" dir="ltr" value="{{ old('isbn', $book->isbn) }}" class="mt-1 w-full rounded border-slate-300">
