...
<main class="mx-auto max-w-7xl px-6 py-10">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold">قرآنيات</h1>
            <p class="mt-1 text-sm text-slate-500">إدارة المقالات والخواطر والفوائد وفيديوهات يوتيوب وملفات PDF لقسم قرآنيات.</p>
        </div>
        @can('permission', 'content.create')
            <a href="{{ route('admin.quraniyat.create') }}" class="rounded-lg bg-emerald-700 px-4 py-2 font-medium text-white">محتوى جديد</a>
        @endcan
    </div>
    ...
    <form method="GET" class="mt-6 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        <div class="min-w-[200px] flex-1"><label for="field-q" class="block text-xs font-medium text-slate-500">بحث</label><input type="text" id="field-q" name="q" value="{{ request('q') }}" placeholder="ابحث بالعنوان أو المحتوى" class="mt-1 w-full rounded border-slate-300"></div>
        <div><label for="field-type" class="block text-xs font-medium text-slate-500">النوع</label>
            <select id="field-type" name="type" class="mt-1 rounded border-slate-300">
                ...
                @foreach($types as $type)<option value="{{ $type }}" @selected(request('type') === $type)>{{ $typeLabels[$type] }}</option>@endforeach
            </select>
        </div>
        ...
        <div class="flex gap-2 pb-0.5"><button class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white">تصفية</button><a href="{{ route('admin.quraniyat.index') }}" class="px-3 py-2 text-sm text-slate-500">إعادة تعيين</a></div>
    </form>
            ...
            <tbody>
                @forelse($items as $item)
                    <tr class="border-t border-slate-200">
                        ...
                        <td class="p-4">{{ $typeLabels[$item->type] ?? $item->type }}</td>
                        ...
                        <td class="p-4 whitespace-nowrap">
                            @if($item->trashed())
                                @can('permission', 'content.update')
                                    <form class="inline" method="POST" action="{{ route('admin.quraniyat.restore', $item->id) }}">@csrf<button class="text-emerald-700">استعادة</button></form>
                                @endcan
                            @else
                                @can('permission', 'content.update')
                                    <a class="text-emerald-700" href="{{ route('admin.quraniyat.edit', $item) }}">تعديل</a>
                                    @if($item->status === 'published')
                                        <form class="inline" method="POST" action="{{ route('admin.quraniyat.unpublish', $item) }}">@csrf<button class="mr-3 text-amber-700">إلغاء النشر</button></form>
                                    @else
                                        <form class="inline" method="POST" action="{{ route('admin.quraniyat.publish', $item) }}">@csrf<button class="mr-3 text-emerald-700">نشر</button></form>
                                    @endif
                                @endcan
                                @can('permission', 'content.delete')
                                    <form class="inline" method="POST" action="{{ route('admin.quraniyat.destroy', $item) }}">@csrf @method('DELETE')<button class="mr-3 text-red-700" onclick="return confirm('حذف هذا المحتوى؟')">حذف</button></form>
                                @endcan
                            @endif
                        </td>
                    </tr>
