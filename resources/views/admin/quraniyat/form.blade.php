...
<main class="mx-auto max-w-4xl px-6 py-10">
    <h1 class="text-2xl font-bold">{{ $item->exists ? 'تعديل محتوى' : 'محتوى جديد' }} — قرآنيات</h1>

    <form method="POST" action="{{ $item->exists ? route('admin.quraniyat.update', $item) : route('admin.quraniyat.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        @csrf
        ...
        <div>
            <label for="field-video_url" class="text-sm font-medium">رابط فيديو يوتيوب (لنوع "فيديو يوتيوب")</label>
        ...
        <fieldset>
            ...
            <p class="mt-1 text-xs text-slate-500">يُضاف هذا المحتوى تلقائيًا إلى قسم «قرآنيات»، ويمكنك اختيار تصنيفات فرعية إضافية.</p>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.quraniyat.index') }}" class="px-4 py-2">إلغاء</a>
        </div>
    </form>
</main>
@endsection
