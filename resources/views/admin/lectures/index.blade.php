...
<main class="mx-auto max-w-7xl px-6 py-10">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold">المحاضرات</h1>
            <p class="mt-1 text-sm text-slate-500">إدارة المحاضرات المرئية.</p>
        </div>
        @can('permission', 'content.create')
            <a href="{{ route('admin.lectures.create') }}" class="rounded-lg bg-emerald-700 px-4 py-2 font-medium text-white">محاضرة جديدة</a>
        @endcan
    </div>
    ...
    <form method="GET" class="mt-6 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div class="flex gap-2 pb-0.5"><button class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white">تصفية</button><a href="{{ route('admin.lectures.index') }}" class="px-3 py-2 text-sm text-slate-500">إعادة تعيين</a></div>
    </form>
                        ...
                        <td class="p-4 whitespace-nowrap">
                            @if($item->trashed())
                                @can('permission', 'content.update')
                                    <form class="inline" method="POST" action="{{ route('admin.lectures.restore', $item->id) }}">@csrf<button class="text-emerald-700">استعادة</button></form>
                                @endcan
                            @else
                                @can('permission', 'content.update')
                                    <a class="text-emerald-700" href="{{ route('admin.lectures.edit', $item) }}">تعديل</a>
                                    @if($item->status === 'published')
                                        <form class="inline" method="POST" action="{{ route('admin.lectures.unpublish', $item) }}">@csrf<button class="mr-3 text-amber-700">إلغاء النشر</button></form>
                                    @else
                                        <form class="inline" method="POST" action="{{ route('admin.lectures.publish', $item) }}">@csrf<button class="mr-3 text-emerald-700">نشر</button></form>
                                    @endif
                                @endcan
                                @can('permission', 'content.delete')
                                    <form class="inline" method="POST" action="{{ route('admin.lectures.destroy', $item) }}">@csrf @method('DELETE')<button class="mr-3 text-red-700" onclick="return confirm('حذف هذه المحاضرة؟')">حذف</button></form>
                                @endcan
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="8" class="p-6 text-center text-slate-500">لا توجد محاضرات مطابقة.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
