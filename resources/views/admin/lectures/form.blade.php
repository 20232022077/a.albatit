...
<main class="mx-auto max-w-4xl px-6 py-10">
    ...
    <form method="POST" action="{{ $item->exists ? route('admin.lectures.update', $item) : route('admin.lectures.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            <div>
                <label class="text-sm font-medium">المحاضِر</label>
                <input name="speaker" value="{{ old('speaker', $lecture->speaker) }}" class="mt-1 w-full rounded border-slate-300">
                @error('speaker')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="text-sm font-medium">تاريخ الإلقاء</label>
                <input type="date" name="delivered_at" value="{{ old('delivered_at', $lecture->delivered_at?->format('Y-m-d')) }}" class="mt-1 w-full rounded border-slate-300">
                @error('delivered_at')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="text-sm font-medium">مكان الإلقاء</label>
                <input name="venue" value="{{ old('venue', $lecture->venue) }}" class="mt-1 w-full rounded border-slate-300">
