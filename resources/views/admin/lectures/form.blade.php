...
<main class="mx-auto max-w-4xl px-6 py-10">
    <h1 class="text-2xl font-bold">{{ $item->exists ? 'تعديل محاضرة' : 'محاضرة جديدة' }}</h1>

    <form method="POST" action="{{ $item->exists ? route('admin.lectures.update', $item) : route('admin.lectures.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        @csrf
        ...
        <div>
            <label for="field-body" class="text-sm font-medium">وصف تفصيلي</label>
            <textarea id="field-body" name="body" rows="6" class="mt-1 w-full rounded border-slate-300">{{ old('body', $item->body) }}</textarea>
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            <div>
                <label for="field-speaker" class="text-sm font-medium">المحاضِر</label>
                <input id="field-speaker" name="speaker" value="{{ old('speaker', $lecture->speaker) }}" class="mt-1 w-full rounded border-slate-300">
                @error('speaker')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-delivered_at" class="text-sm font-medium">تاريخ الإلقاء</label>
                <input type="date" id="field-delivered_at" name="delivered_at" value="{{ old('delivered_at', $lecture->delivered_at?->format('Y-m-d')) }}" class="mt-1 w-full rounded border-slate-300">
                @error('delivered_at')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-venue" class="text-sm font-medium">مكان الإلقاء</label>
                <input id="field-venue" name="venue" value="{{ old('venue', $lecture->venue) }}" class="mt-1 w-full rounded border-slate-300">
                @error('venue')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.lectures.index') }}" class="px-4 py-2">إلغاء</a>
        </div>
    </form>
</main>
@endsection
