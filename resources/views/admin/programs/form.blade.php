...
<main class="mx-auto max-w-4xl px-6 py-10">
    ...
    <form method="POST" action="{{ $item->exists ? route('admin.programs.update', $item) : route('admin.programs.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            <div>
                <label for="field-presenter" class="text-sm font-medium">المقدّم</label>
                <input id="field-presenter" name="presenter" value="{{ old('presenter', $program->presenter) }}" class="mt-1 w-full rounded border-slate-300">
                @error('presenter')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-started_on" class="text-sm font-medium">تاريخ البداية</label>
                <input type="date" id="field-started_on" name="started_on" value="{{ old('started_on', $program->started_on?->format('Y-m-d')) }}" class="mt-1 w-full rounded border-slate-300">
                @error('started_on')<p class="mt-1 text-sm text-red-700">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="field-ended_on" class="text-sm font-medium">تاريخ النهاية</label>
                <input type="date" id="field-ended_on" name="ended_on" value="{{ old('ended_on', $program->ended_on?->format('Y-m-d')) }}" class="mt-1 w-full rounded border-slate-300">
