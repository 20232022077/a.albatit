...
<main class="mx-auto max-w-4xl px-6 py-10">
    <h1 class="text-2xl font-bold">{{ $item->exists ? 'تعديل محتوى' : 'محتوى جديد' }} — مركزية القرآن</h1>

    <form method="POST" action="{{ $item->exists ? route('admin.quran-centrality.update', $item) : route('admin.quran-centrality.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        @csrf
        @if($item->exists) @method('PUT') @endif

        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label for="field-type" class="text-sm font-medium">نوع المحتوى</label>
                <select id="field-type" name="type" class="mt-1 w-full rounded border-slate-300">
                    @foreach($types as $type)<option value="{{ $type }}" @selected(old('type', $item->type) === $type)>{{ $typeLabels[$type] }}</option>@endforeach
                </select>
        ...
        <div>
            <label for="field-excerpt" class="text-sm font-medium">الوصف المختصر</label>
        ...
        <div>
            <label for="field-body" class="text-sm font-medium">المحتوى</label>
        ...
        <div>
            <label for="field-video_url" class="text-sm font-medium">رابط الفيديو (اختياري لنوع الفيديو)</label>
            <input id="field-video_url" name="video_url" dir="ltr" value="{{ old('video_url', $item->meta['video_url'] ?? '') }}" placeholder="https://" class="mt-1 w-full rounded border-slate-300">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            ...
            <div class="flex items-end pb-2">
                <label class="flex items-center gap-2"><input type="hidden" name="is_featured" value="0"><input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $item->is_featured))> محتوى مميز</label>
            </div>
        </div>

        <fieldset>
            <legend class="text-sm font-medium">التصنيفات الفرعية</legend>
            <p class="mt-1 text-xs text-slate-500">يُضاف هذا المحتوى تلقائيًا إلى قسم «مركزية القرآن»، ويمكنك اختيار تصنيفات فرعية إضافية.</p>
            <div class="mt-2 grid gap-2 sm:grid-cols-2">
                ...
                @empty
                    <p class="text-sm text-slate-400">لا توجد تصنيفات فرعية بعد.</p>
                @endforelse
            </div>
        </fieldset>

        <div>
            <label for="field-tags" class="text-sm font-medium">الوسوم (مفصولة بفواصل)</label>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.quran-centrality.index') }}" class="px-4 py-2">إلغاء</a>
        </div>
    </form>
</main>
@endsection
