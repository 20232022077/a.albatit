...
<main class="mx-auto max-w-4xl px-6 py-10">
    ...
    <form method="POST" action="{{ $item->exists ? route('admin.quran-centrality.update', $item) : route('admin.quran-centrality.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label class="text-sm font-medium">نوع المحتوى</label>
                <select name="type" class="mt-1 w-full rounded border-slate-300">
        ...
        <div>
            <label class="text-sm font-medium">الوصف المختصر</label>
        ...
        <div>
            <label class="text-sm font-medium">المحتوى</label>
        ...
        <div>
            <label class="text-sm font-medium">رابط الفيديو (اختياري لنوع الفيديو)</label>
            <input name="video_url" dir="ltr" value="{{ old('video_url', $item->meta['video_url'] ?? '') }}" placeholder="https://" class="mt-1 w-full rounded border-slate-300">
