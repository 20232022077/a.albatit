...
<main class="mx-auto max-w-7xl px-6 py-10">
    ...
    <form method="GET" class="mt-6 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        <div class="min-w-[200px] flex-1"><label class="block text-xs font-medium text-slate-500">بحث</label><input type="text" name="q" value="{{ request('q') }}" placeholder="ابحث بالعنوان أو النص" class="mt-1 w-full rounded border-slate-300"></div>
                        ...
                        <td class="p-4 whitespace-nowrap">
                            ...
                            @else
                                ...
                                @can('permission', 'content.delete')
                                    <form class="inline" method="POST" action="{{ route('admin.reflections.destroy', $item) }}">@csrf @method('DELETE')<button class="mr-3 text-red-700" onclick="return confirm('حذف هذا التأمل؟')">حذف</button></form>
                                @endcan
                            @endif
                        </td>
                    </tr>
