...
@php
    ...
    $metaDescription = 'تأملات قرآنية قصيرة تقرّب معاني الآيات إلى القلب.';
...
<main class="mx-auto max-w-6xl px-5 py-12">
    ...
    <h1 class="mt-5 text-3xl font-bold">التأملات</h1>
    ...
    <div class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        @forelse($items as $item)
            <a href="{{ route('reflections.show', $item) }}" class="block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
        ...
        @empty
            <p class="col-span-full rounded-xl bg-slate-100 p-8 text-center text-slate-500">لا توجد تأملات منشورة حاليًا.</p>
        @endforelse
    </div>
